'use strict';
mainApp.controller('transactionInfoPunishmentController',
[
    ...
    function($scope, $state, transactionService, Notification, TransactionType, TransactionState, ConfigData) {
        ...
        $scope.loadData = function(tableState) {
            ...
            $scope.data = [];
            transactionService.queryUserPunishment(JSON.stringify(tableState),
                function(response) {
        ...
        $scope.viewDefails = function(target) {
            ...
            $state.go('transaction.punishment.details', { key: target.id });
        }
